<script>
    ...
    import  { open as drawerOpen } from '../../../../stores/drawer';
...
<div transition:fade={{duration: 150}} class="h-16 flex border-primary" style={`width: calc(100% - ${item.depth * 48}px)`}>
    <a class="underline hover:text-gray-600 cursor-pointer pl-8 flex-left w-full text-xl" on:click={handleDrawerClose} href={item.url}>{item.name}</a>
